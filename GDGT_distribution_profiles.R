...
create_barplot <- function(data, title_suffix) {
  ...
  # Reshape data into a matrix, dropping any rows with NAs in the summarization
  matrix <- data %>%
    select(Site, iGDGT.0, iGDGT.1, iGDGT.2, iGDGT.3, iGDGT.4, iGDGT.5, iGDGT.6, iGDGT.7, iGDGT.8) %>%
    group_by(Site) %>%
    summarise(across(starts_with("iGDGT"), ~ mean(.x, na.rm = TRUE))) %>%
    ungroup() %>%
    filter(if_all(starts_with("iGDGT"), ~ !is.na(.))) %>%
    column_to_rownames(var = "Site") %>%
    t()  # Transpose to get iGDGTs as rows and sites as columns
  
  # Check the dimensions of the reshaped matrix
  print(dim(matrix))
  print(matrix)
  ...
  if (ncol(matrix) > 0) {
    ...
    # Set y-axis limits based on the max of the normalized matrix
    ylim <- c(0, max(matrix, na.rm = TRUE) * 1.1)  # Set limits slightly above max
    
    # Correctly adjust the X-axis labels to pH values
    num_sites <- ncol(matrix)
    axis(1, at = seq(0.5, num_sites + 0.5, length.out = num_sites),
         las = 1,
         labels = data$pH[1:num_sites],  # Change to correspond with pH values
         tck = -0.035,
         cex.axis = 1)
